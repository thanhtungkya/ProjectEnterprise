...
router.post("/", requireAuth, async (req: Request, res: Response) => {
  const { username, password, name, role, departmentId, avatarUrl } = req.body;
  ...
  try {
    ...
    const insertUser = await query(
      "INSERT INTO users (username, password_hash, department_id, avatar_url, created_at, updated_at) VALUES ($1, $2, $3, $4, NOW(), NOW()) RETURNING *",
      [username, hashedPassword, departmentId || null, avatarUrl || null],
    );
    ...
    if (roleRow.rows.length) {
      await query(
        ...
        [createdUser.id, roleRow.rows[0].id],
      );
    }
...
router.patch("/:id", requireAuth, async (req: Request, res: Response) => {
  const { username, password, departmentId, role, is_deleted, avatarUrl } =
    req.body;
  const fields: string[] = [];
  const values: any[] = [];
  ...
  try {
    ...
    if (role) {
      ...
      if (roleRow.rows.length) {
        await query("DELETE FROM user_roles WHERE user_id = $1", [
          req.params.id,
        ]);
        await query(
          "INSERT INTO user_roles (user_id, role_id) VALUES ($1, $2) ON CONFLICT DO NOTHING",
          [req.params.id, roleRow.rows[0].id],
        );
      }
    }
